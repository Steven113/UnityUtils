...
namespace Assets.Scripts
{
    ...
    {
        public static IEnumerator IterateTransformPositionCoroutine(Transform transformToMove, IEnumerable<Transform> targets, float rate, float positionReachedTolerance)
        {
            foreach (var target in targets)
            {
                while (Vector3.Distance(target.transform.position, transformToMove.transform.position) > positionReachedTolerance)
                {
                    transformToMove.transform.position = Vector3.MoveTowards(transformToMove.transform.position, target.transform.position, rate * Time.deltaTime);
